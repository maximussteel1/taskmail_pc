...
def build_status_subject(status_label: str, subject_text: str) -> str:
    subject = subject_text.strip()
    return f"[{status_label}] {subject}".rstrip()
...
def _build_capsule_state(
    ...
    return {
        ...
        "task_id": state_dict.get("current_task_id") or state_dict.get("task_id", ""),
    ...
    *,
    ...
    lines.extend(
        [
            ...
            f"Task ID: {state_dict.get('current_task_id') or state_dict.get('task_id', '')}",
    ...
    if state_dict.get("last_summary"):
