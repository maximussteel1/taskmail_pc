...
from .session_semantics import thread_monitor_should_stay_open
...
THREAD_CLOSE_REQUESTS_DIRNAME = "thread_close_requests"
...
def thread_close_requests_dir(runtime_dir: str | Path) -> Path:
    return Path(runtime_dir).resolve() / THREAD_CLOSE_REQUESTS_DIRNAME
...
def write_thread_close_request(
    runtime_dir: str | Path,
    *,
    thread_id: str,
    task_id: str,
    ...
    request_dir = thread_close_requests_dir(runtime_dir)
    request_dir.mkdir(parents=True, exist_ok=True)
    normalized_requested_at = requested_at or _timestamp()
    request_id = f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_{secrets.token_hex(2)}"
    payload = {
...
def list_thread_close_request_paths(runtime_dir: str | Path) -> list[Path]:
    request_dir = thread_close_requests_dir(runtime_dir)
    if not request_dir.exists():
        return []
    return sorted(path for path in request_dir.glob("*.json") if path.is_file())
...
def read_thread_close_request(path: str | Path) -> dict[str, str]:
    payload = json.loads(Path(path).read_text(encoding="utf-8-sig"))
    if not isinstance(payload, dict):
        raise ValueError("Thread close request must be a JSON object.")
    ...
    if not normalized["thread_id"]:
        raise ValueError("Thread close request is missing thread_id.")
    if not normalized["task_id"]:
        raise ValueError("Thread close request is missing task_id.")
    if not normalized["request_id"]:
        normalized["request_id"] = Path(path).stem
    if not normalized["source"]:
        normalized["source"] = "pc"
...
def request_thread_close(
    *,
    ...
    return 0, f"Queued local close request for thread {thread_id} task {task_id}: {request_path}"
...
def main(argv: list[str] | None = None) -> int:
    ...
    request_close.add_argument("--task-root", help="Optional task root override.")
    request_close.add_argument("--config", help="Optional config path used to resolve task_root.")
    request_close.add_argument("--source", default="pc", help="Operator-facing source label for logging.")
    ...
    if args.command == "request-thread-kill":
        ...
        print(message)
        return exit_code
    if args.command == "request-thread-close":
        exit_code, message = request_thread_close(
            runtime_dir=args.runtime_dir,
            thread_id=args.thread_id,
            task_root=args.task_root,
            config_path=args.config,
