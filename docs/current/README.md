...
- Focused monitor windows now run under a small hidden controller. If the focused window is interrupted with `Ctrl+C`, closed with `X`, or its terminal tab/pane is closed while the session is still monitorable, the controller queues a local close request that kills the current run if needed and then marks the session `ended` once it is no longer running. Overview windows still close without changing backend state.
...
- On Windows, setting `spawn_monitor_windows: true` now opens one focused monitor window per running thread. These auto-opened windows now launch through `scripts/monitor_mail_runner_controller.ps1`, still focus `follow-thread-live <thread_id>`, stay open while that session remains active/resumable, and honor `monitor_window_buffer_lines` plus `monitor_window_history_limit` so the console scrollback and startup replay stay bounded.
