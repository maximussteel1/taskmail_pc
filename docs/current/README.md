...
- repo-side 现在还额外存在一个薄的 Android-facing facade 入口：`POST /v1/android/create-session`。它同样复用当前 relay host/port 与同一 `Authorization: Bearer <transport_token>` admission，把 Android-facing `CreateSessionCommand` 薄映射到内部 `pc_control_runtime -> command_dispatch(new_task)`，并在同一提交窗口返回 `command_id + submit_ack`，当 `ack_status=accepted|accepted_but_queued` 时还会一并返回 `session_binding(session_id/pc_id/workspace_id)`。这条入口当前应读成 repo-side `VPS-first` facade seam，而不是把 [android_runner_communication_contract.md](./android_runner_communication_contract.md) 所定义的 current Android mail-first boundary 直接改写成“已经整体切到新 app API”
