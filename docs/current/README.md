...
- The active working set is capped at `4` sessions.
...
- 当前大文件交付支持 COS 外链路径：超阈值 artifact 会保留在 `Artifacts` 区域，并额外出现在 `External Deliveries` 区域，而不是继续作为超大附件投递
